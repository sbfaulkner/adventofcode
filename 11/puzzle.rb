...
PASSWORD = File.read(INPUT_PATH).chomp
...
time = Benchmark.realtime do
  answer = Password.new(PASSWORD).next.value
end

STDERR.printf "Part 1: answer=%s (%.3fms elapsed)\n", answer, time * 1000

time = Benchmark.realtime do
  answer = Password.new(answer).next.value
end

STDERR.printf "Part 2: answer=%s (%.3fms elapsed)\n", answer, time * 1000
