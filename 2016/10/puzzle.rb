...
# previous sequence and using that reading as the next sequence. For example, 211 is read as "one two, two ones", which
...
DIGITS = File.read(INPUT_PATH).chomp
...
time = Benchmark.realtime do
  digits = DIGITS
  40.times { digits = Player.new(digits).say }
  answer = digits.length
end
...
time = Benchmark.realtime do
  digits = DIGITS
  50.times { digits = Player.new(digits).say }
  answer = digits.length
end
