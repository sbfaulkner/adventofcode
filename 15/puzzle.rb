...
time = Benchmark.realtime do
  ingredients = Ingredient.load(File.open(INPUT_PATH))
  recipes = Recipe.combine(ingredients)
  answer = recipes.map(&:score).max
end
...
time = Benchmark.realtime do
  ingredients = Ingredient.load(File.open(INPUT_PATH))
  recipes = Recipe.combine(ingredients)
  answer = recipes.select { |recipe| recipe.calories == 500 }.map(&:score).max
end
